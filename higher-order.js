...
let evenNumbers = mixedNumbers.filter( (elm) => {
  return elm % 2 === 0;
})
...
let postTaxPrices = prices.map((elm) => {
  return elm * 1.07;
});
...
let totalPopulation = populations.reduce( (acc, elm) => {
  return acc + elm;
}, 0)
...
let myStrongest = monstersInYourPocket.filter( (elm) => {
  return elm.CP > 200;
})
...
let orderTotals = orders.map( (elm) => {
  let arr = [];
  let tax = elm.price * elm.tax;
  let sum = tax + elm.price;
  arr.push(sum);
  return arr;
})
...
let bobsTotal = purchases.reduce( (acc, elm) => {
  let total = 0;
  if (elm.owner === "Bob"){
    total = elm.price
  }
